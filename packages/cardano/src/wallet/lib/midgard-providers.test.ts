...
describe('MidgardUtxoProvider', () => {
  ...
  describe('utxoByAddresses', () => {
    it('should transform Blockfrost-style UTxO data to Cardano SDK format', async () => {
      ...
      // Mock Blockfrost-style UTxO response
      ...
      expect(txOut.value.coins).toBe(BigInt(4027026465));
    ...
    it('should handle UTxOs with only lovelace (no assets)', async () => {
      ...
      expect(txOut.value.coins).toBe(BigInt(9825963));
      expect(txOut.value.assets).toBeInstanceOf(Map); // Should be an empty Map when no assets
      expect((txOut.value.assets as Map<any, any>).size).toBe(0); // Should be empty
    });
    ...
    it('should handle multiple addresses', async () => {
      ...
      mockMidgardClient.request = jest.fn()
    ...
    it('should log debug messages', async () => {
      ...
      expect(mockLogger.debug).toHaveBeenCalledWith(
        expect.stringContaining('Fetching UTxOs from Midgard for 1 addresses')
      );
    });
