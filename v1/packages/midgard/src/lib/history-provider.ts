 ...
 */
const globalUtxoSet: Map<string, string> = new Map(); // address -> utxo hash
...
export class MidgardChainHistoryProvider implements ChainHistoryProvider {
  ...
  private createDummyTransaction(): Cardano.HydratedTx {
    ...
    // eslint-disable-next-line no-magic-numbers
    const txId = `${now.toString(16).padStart(32, '0')}${randomHex}`.slice(0, 64);
    ...
    for (const address of addresses) {
      try {
        ...
        if (previousHash && previousHash !== currentHash) {
          ...
          this.logger.info(
            `[MidgardChainHistoryProvider] Added dummy transaction for UI refresh for address ${address}`
          );
        }
  ...
  async transactionsByAddresses(args: TransactionsByAddressesArgs): Promise<Paginated<Cardano.HydratedTx>> {
    ...
    try {
      ...
      if (dummyTransactions.length > 0) {
        this.logger.info(
          `[MidgardChainHistoryProvider] Generated ${dummyTransactions.length} dummy transactions for UI refresh`
        );
        return {
